...
const totalLanguages = setOfLanguages.size
console.log("Total languages:", totalLanguages)

// Use the countries data to find the 10 most spoken languages:
const mostSpokenLanguages = (languages, number = 10) => {
    // let count = 0
    const counts = []

    for (const language of languages) {
        const filteredLanguage = languages.filter((lang) => lang === language)
        const sortedLanguage = filteredLanguage.sort((language1, language2) => language2.length - language1.length)
        // console.log(sor)
        counts.push({ language: language, count: sortedLanguage.length})
    }
